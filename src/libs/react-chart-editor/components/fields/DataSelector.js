...
export class UnconnectedDataSelector extends Component {
  ...
  setLocals(props, context) {
    ...
    if (props.originalTraceContainer) {
      ...
      const options = nestedProperty(props.originalTraceContainer, 'options').get() || [];
      const values = nestedProperty(props.originalTraceContainer, 'possibleValues').get() || {};
      this.setState({
        selectedSqlBlock,
        options,
        values
      ...
      const options = nestedProperty(props.container, 'options').get() || [];
      const values = nestedProperty(props.container, 'possibleValues').get() || {};
      this.setState({
        options, values
      });
    }
  ...
  updatePlot(value) {
    ...
    if (Array.isArray(adjustedValue)) {
      ...
      if (this.state.values) {
        data = this.state.values[adjustedValue] || null;
      } else {
  ...
  render() {
    ...
    const {selectedSqlBlock} = this.state;
    let newLabel;
    ...
    let tooltip = null;
    if (showTooltip && selectedSqlBlock) {
      tooltip = `Datasource: ${selectedSqlBlock.name}`;
    }

    return (
      <Field {...{...this.props, label: newLabel, tooltip}}>
        <DropdownWidget
          options={this.state.options || this.dataSourceOptions}
...
UnconnectedDataSelector.propTypes = {
  ...
  selectedSqlBlock: PropTypes.any,
