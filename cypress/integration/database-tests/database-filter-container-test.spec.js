describe('Database - Filter - Container', () => {
  before(() => {
    cy.visit('/d');
  });
  beforeEach(() => {
  ...
  it('should shows Database Name option after click on add filter', () => {
    cy.get('[data-cy=filterListTagsToolbar]').find('.bp3-button-text').last().click();
    cy.get('[data-cy=filterItems]').find('.bp3-text-overflow-ellipsis').first().should('have.text', 'Database Name');
  });

  it('should shows Database Name tag and filter options after click on Database Name filter', () => {
  ...
  it('should shows Apply button in Database filter options', () => {
    cy.get('[data-cy=selectedFilterOptions]').find('.filters__menu__apply').find('span').last().should('have.text', 'Apply');
  });

  it('should shows an input and auto-focus on it when select one of the Database filter options', () => {
  ...
  it(`should shows a tag with text 'Database Name is test' and a remove button`, () => {
    cy.get('[data-cy=selectedFilterOptions]').find('.custom-radio-button__input').first().type('test{enter}');
    cy.get('[data-cy=filterTags]').find('.bp3-text-overflow-ellipsis').first().should('have.text',`Database Name is 'test'`);
    cy.get('[data-cy=filterTags]').find('.bp3-tag-remove').first().should('exist');
  });

  it('should remove the filter tag after click on remove button in Database filter tags', () => {
    cy.get('[data-cy=filterTags]').find('.bp3-tag-remove').first().click();
    cy.get('[data-cy=filterTags]').should('not.exist');
  });

  it('should shows a popup after click on save view in Database List View page with Create a new view header and a close button', () => {
  ...
  it('Database Save view popup should have body and footer', () => {
    cy.get('.bp3-dialog').find('.bp3-dialog-body').first().should('exist');
    cy.get('.bp3-dialog').find('.bp3-dialog-footer').first().should('exist');
  });

  it('Database Save view popup body should have an input with Name label and should auto-focus on the input', () => {
  ...
  it('Database Save view popup footer should have Cancel and Create buttons', () => {
  ...
  it('Database Save view popup footer should disappear after click on cancel button ', () => {
